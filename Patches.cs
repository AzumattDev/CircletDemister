...
static class ZNetSceneAwakePatch
{
    ...
    {
        CircletDemisterPlugin.DemisterParticleFf =
            __instance.GetPrefab("Demister").GetComponentInChildren<ParticleSystemForceField>();
...
static class PlayerEquipItemPatch
{
        ...
        bool triggerEquipEffects = true)
    {
        ...
        {
            CircletDemisterPlugin.PlayerParticleFf = Object.Instantiate(CircletDemisterPlugin.DemisterParticleFf,
                __instance.GetHeadPoint(), Quaternion.identity, __instance.transform);
        }
    }
}
...
static class PlayerUnequipItemPatch
{
        ...
        bool triggerEquipEffects = true)
    {
        ...
        if (item?.m_dropPrefab.name != "HelmetDverger" || Player.m_localPlayer == null || !__instance.IsPlayer() ||
            Player.m_localPlayer != __instance) return;
